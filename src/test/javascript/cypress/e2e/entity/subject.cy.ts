...
describe('Subject e2e test', () => {
  ...
  const subjectSample = { language: 'ENGLISH', title: 'turn-key SAS Czech' };
  ...
  describe('new Subject page', () => {
    ...
    it('should create an instance of Subject', () => {
      cy.get(`[data-cy="age"]`).select('JUNIOR');

      cy.get(`[data-cy="language"]`).select('KIKUYU');

      cy.get(`[data-cy="title"]`).type('Investor').should('have.value', 'Investor');
